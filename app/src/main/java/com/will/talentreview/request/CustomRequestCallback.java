...
public abstract class CustomRequestCallback<T> extends ResponseCallback<String, ResponseBody> {

    private Gson gson;
    private JsonParser jsonParser;

    public CustomRequestCallback() {
        gson = new Gson();
    ...
    public void onNext(Object tag, Call call, String response) {
        ...
        onRequestFinished(getRequestResult(response, entityClass));
    }
            ...
            if (cla == String.class) {
                ...
                if (listArray instanceof JSONArray) {
                    ...
                    if (((JSONObject) listArray).has("list")) {
                        ...
                        listBean.setCurrPage(data.optInt("currPage", 0));
                        listBean.setPageSize(data.optInt("pageSize", 0));
                        listBean.setTotalCount(data.optInt("totalCount", 0));
                        listBean.setTotalPage(data.optInt("totalPage", 0));
